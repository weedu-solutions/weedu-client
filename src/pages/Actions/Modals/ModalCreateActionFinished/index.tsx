...
import { Modal, ModalButtonContainer, ModalCancelButton, ModalErrorMessage, ModalGrid, ModalSection, ModalSectionTitle, ModalSubmitButton } from "../../../../components/Modal";
...
export function ModalCreateActionFinished({
  closeModal,
  ...
  const init_date = watch("init_date");
  const end_date = watch("end_date");

  const isPreviewDateValid =
    preview_end_date &&
    preview_init_date &&
    preview_end_date >= preview_init_date;

  const isRealDateValid =
    end_date && init_date && end_date >= init_date;
  ...
  const onSubmit = async (data: CreateActionFormData) => {
    try {
      ...
      await createAction.mutateAsync({
        ...
        init_date: moment(data.init_date).format("DD/MM/YYYY"),
  ...
  return (
    <Modal
      title="Nova Ação Executada"
      subtitle="Preencha os dados para criar uma nova ação Executada"
      onClose={closeModal}
    >
      <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <FormSelect
              ...
              onChange={(e) => {
                ...
                if (selectedOption) {
                  ...
                  setValue("user_id", selectedOption.value.toString());
                }
              }}
            />
          </ModalGrid>
        ...
        <ModalSection>
          ...
          {preview_end_date && preview_init_date && !isPreviewDateValid && (
            <ModalErrorMessage>
        ...
        <ModalSection>
          <ModalGrid>
            <FormInput
              ...
              type="date"
              register={register}
              error={errors.init_date?.message}
            />
          ...
          {end_date && init_date && !isRealDateValid && (
            <ModalErrorMessage>
              A data final real deve ser maior ou igual à data inicial real
            </ModalErrorMessage>
          )}
        </ModalSection>
        ...
        <ModalButtonContainer>
          ...
          <ModalSubmitButton 
            type="submit" 
            disabled={loading || !isPreviewDateValid || !isRealDateValid}
          >
