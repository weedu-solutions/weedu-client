...
export function UpdateConsultant() {
  ...
  const [profileType, setProfileType] = useState(
    consultant ? consultant?.user_type_id : Number(3)
  );
  ...
  const onSubmit = async ({ name, suname, email, phone }: IConsultant) => {
        ...
        .then((res: AxiosResponse) => {
          // eslint-disable-next-line no-lone-blocks
          {
            consultant?.is_active === 1
              ...
              : Notify(
                  NotifyTypes.SUCCESS,
                  "Consultor desbloqueado com sucesso!"
                );
          }
          navigate(ROUTES.CONSULTANTS);
        })
        .catch((err: AxiosResponse) => {
          // eslint-disable-next-line no-lone-blocks
          {
            consultant?.is_active === 1
              ? Notify(
                  NotifyTypes.ERROR,
                  "Não foi possível bloquear o consultor."
                )
              : Notify(
                  NotifyTypes.ERROR,
                  "Não foi possível desbloquear o consultor."
                );
          }
        });
    } else {
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <ButtonWrapper>
            <ButtonBlock>
              <ButtonDefault
                backgroundColor={
                  consultant?.is_active === 0 ? "#E71D36" : "#7956F7"
                }
                width={"40%"}
                height={"50px"}
      ...
      <Modal isOpen={isModalBlockOpen} style={customStyleModalBlock}>
        <ModalBody>
          <ModalTitle>
            <h1>
              Confirmação de{" "}
              {consultant?.is_active === 1 ? "bloqueio" : "desbloqueio"}
            </h1>
          </ModalTitle>
          ...
          <ModalSubtitle>
            <p>
              ...
              {consultant?.is_active === 1 ? "bloquear " : "desbloquear "}
            </p>
            <span>o consultor(a) {consultant?.name} ?</span>
          </ModalSubtitle>
