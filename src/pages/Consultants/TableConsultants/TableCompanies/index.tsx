...
export function TableCompanies({ userRow }: ITableCompanies) {
  const [isModalLinkCompanies, setIsModalLinkCompanies] =
    useState<boolean>(false);
  const [isModalWithdrawCompanies, setIsModalWithdrawCompanies] =
    useState<boolean>(false);
  const [idCompanie, setIdCompanie] = useState<number>();

  const { data } = useFetch<any>(
    `/auth/consultant-with-customer/${userRow.id}`
  );
  ...
  if (data) {
    return (
      <>
        <Modal isOpen={isModalLinkCompanies} style={styleModalLinkCompanies}>
          <>
            <ModalLinkCompanies
